<template>
  <div>
    ...
    <!-- Error State -->
    <div v-else-if="error" class="bg-red-50 border border-red-200 rounded-lg p-6 mb-6 dark:bg-red-950 dark:border-red-900">
      <div class="flex items-center">
        <svg class="w-6 h-6 text-red-600 mr-3 dark:text-red-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 8v4m0 4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z"></path>
        </svg>
        <div>
          <h3 class="text-red-800 font-medium dark:text-red-200">Error loading IP information</h3>
          <p class="text-red-600 text-sm mt-1 dark:text-red-300">{{ error.message }}</p>
        </div>
      </div>
    ...
    <div v-else-if="data?.success && data.data" class="grid grid-cols-1 md:grid-cols-2 gap-6">
      <!-- Main IP Card -->
      <div class="bg-white rounded-xl shadow-lg p-6 md:col-span-2 dark:bg-slate-900 dark:shadow-none dark:ring-1 dark:ring-slate-800">
      ...
      <!-- Location Card -->
      <div class="bg-white rounded-xl shadow-lg p-6 dark:bg-slate-900 dark:shadow-none dark:ring-1 dark:ring-slate-800">
        <h3 class="text-xl font-semibold text-gray-800 mb-4 flex items-center dark:text-slate-100">
        ...
        <div class="space-y-3">
          <div class="flex justify-between">
            ...
            <span class="font-medium dark:text-slate-100">{{ data.data.country }}</span>
          </div>
          <div class="flex justify-between">
            <span class="text-gray-600 dark:text-slate-300">Region:</span>
            <span class="font-medium dark:text-slate-100">{{ data.data.regionName }}</span>
          </div>
          <div class="flex justify-between">
            <span class="text-gray-600 dark:text-slate-300">City:</span>
            <span class="font-medium dark:text-slate-100">{{ data.data.city }}</span>
          </div>
          <div class="flex justify-between">
            <span class="text-gray-600 dark:text-slate-300">Timezone:</span>
            <span class="font-medium dark:text-slate-100">{{ data.data.timezone }}</span>
          </div>
        </div>
      </div>
      ...
      <div class="bg-white rounded-xl shadow-lg p-6 dark:bg-slate-900 dark:shadow-none dark:ring-1 dark:ring-slate-800">
        ...
        <div class="space-y-3">
          <div class="flex justify-between">
            ...
            <span class="font-medium text-sm dark:text-slate-100">{{ data.data.isp }}</span>
          </div>
          <div v-if="data.data.as" class="flex justify-between">
            <span class="text-gray-600 dark:text-slate-300">AS:</span>
            <span class="font-medium text-sm dark:text-slate-100">{{ data.data.as }}</span>
          </div>
          <div class="flex justify-between">
            <span class="text-gray-600 dark:text-slate-300">Latitude:</span>
            <span class="font-medium dark:text-slate-100">{{ data.data.lat }}°</span>
          </div>
          <div class="flex justify-between">
            <span class="text-gray-600 dark:text-slate-300">Longitude:</span>
            <span class="font-medium dark:text-slate-100">{{ data.data.lon }}°</span>
          </div>
        </div>
      </div>
